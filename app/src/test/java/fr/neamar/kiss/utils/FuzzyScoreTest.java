...
class FuzzyScoreTest {
    ...
    public void testOperations(String query, String testString, int score, boolean match) {
        ...
        FuzzyScore.MatchInfo result = doFuzzy(queryNormalized.codePoints, testStringNormalized.codePoints);
    ...
    private FuzzyScore.MatchInfo doFuzzy(int[] query, int[] testString) {
        return createFuzzyScore(query)
                .match(testString);
    }

    private FuzzyScore createFuzzyScore(int[] query) {
        return new FuzzyScore(query, true)
